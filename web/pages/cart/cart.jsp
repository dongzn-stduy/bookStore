	...
	<div id="main">
		...
		<table>
			...
			<c:if test="${empty sessionScope.cart.items}">
				<%--如果购物车空的情况--%>
				<tr>
					<td colspan="5"><a href="index.jsp">当前购物车为空，去添加商品吧</a>
					</td>
				</tr>
			</c:if>
			<c:if test="${not empty sessionScope.cart.items}">
				...
				<c:forEach items="${sessionScope.cart.items}" var="entry">
					<tr>
						...
						<td><a class="deleteItem" href="cartServlet?action=deleteItem&id=${entry.value.id}">
							删除</a></td>
					</tr>
				</c:forEach>
			</c:if>
		...
		<c:if test="${not empty sessionScope.cart.items}">
			<div class="cart_info">
				...
				<span class="cart_span">总金额<span class="b_price">
						${sessionScope.cart.totalPrice}</span>元</span>
